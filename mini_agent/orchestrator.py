...
class Orchestrator:
    ...
    def create(self, spec: ResourceSpec):
        return self.codegen.generate_module(spec)
    ...
    def validate(self, module_name: str) -> dict:
        module_dir = self.base_dir / "generated" / "backend" / module_name
        required_base = ["__init__.py", "schemas.py", "crud.py", "router.py", "metadata.json"]
        ...
        if not module_dir.exists():
            result["missing_files"] = required_base
            return result
        ...
        if "public" in access_modes:
            required.append("router_public.py")
        if any(x in access_modes for x in ("auth", "rbac")):
            required.append("router_secure.py")
