...
class BackendCodegenAgent:
    ...
    def generate_module(self, spec_input, output_dir: Optional[str] = None) -> list[Path]:
    ...
    def _generate_core(self) -> list[Path]:
        ...
        generated.append(self._write_file(self.base_dir / ".env", self.template_engine.render_env()))
        return self._dedupe_paths(generated)
    ...
    def _generate_resource(self, spec: ResourceSpec) -> list[Path]:
        generated: list[Path] = []
        ...
        primary_mode = "rbac" if "rbac" in spec.access_modes else "auth" if "auth" in spec.access_modes else "public"
