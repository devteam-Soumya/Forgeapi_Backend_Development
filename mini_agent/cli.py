...
def handle_validate(args) -> None:
    orchestrator = Orchestrator(base_dir=args.base_dir)
    result = orchestrator.validate(args.module_name.strip().lower())
    print(json.dumps(result, indent=2))


def handle_list(args) -> None:
    orchestrator = Orchestrator(base_dir=args.base_dir)
    print(json.dumps(orchestrator.list_modules(), indent=2))
...
def build_parser() -> argparse.ArgumentParser:
    ...
    validate_parser = subparsers.add_parser("validate", help="Validate generated module files")
    validate_parser.add_argument("--module-name", required=True, help="Resource/module name")
